...
const invocationName = "cat society";
...
const BudgetIntent_Handler =  {
    canHandle(handlerInput) {
        const request = handlerInput.requestEnvelope.request;
        return request.type === 'IntentRequest' && request.intent.name === 'BudgetIntent' ;
    },
    handle(handlerInput) {
        const request = handlerInput.requestEnvelope.request;
        const responseBuilder = handlerInput.responseBuilder;
        ...
        let say = 'Hello from BudgetIntent. ';
...
const EventIntent_Handler =  {
    canHandle(handlerInput) {
        const request = handlerInput.requestEnvelope.request;
        return request.type === 'IntentRequest' && request.intent.name === 'EventIntent' ;
    },
    async handle(handlerInput) {
        ...
        let say = 'Hello from EventIntent. ';
        ...
        //   SLOT: type
        if (slotValues.type.heardAs && slotValues.type.heardAs !== '') {
        ...
        if( (slotValues.type.ERstatus === 'ER_SUCCESS_NO_MATCH') ||  (!slotValues.type.heardAs) ) {
           // slotStatus += 'A few valid values are, ' + sayArray(getExampleSlotValues('EventIntent','type'), 'or');
        }

        say += slotStatus;


        say = await new Promise((resolve, reject) => {
          setTimeout(() => {
            let say = 'You can do one of these events: ';
            if(slotValues.type.heardAs === 'funtastic') {
              say += sayArray(['Go Kart', 'Climbing', 'Laser Tag', 'Paint Ball'])
            } else if(slotValues.type.heardAs === 'dinner') {
              say += sayArray(['Sushi', 'Burger', 'Schnitzel', 'Pizza'])
                say += ' ... <emphasis level="strong">so</emphasis> <prosody rate="40%">good</prosody>. <emphasis level="moderate">Yummy!</emphasis>!'
            } else {
              say = 'I do not know what you mean.'
            }
            resolve(say)
          }, 3000)
        })
